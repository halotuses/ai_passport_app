...
private extension IncorrectAnswerView {
    @ViewBuilder
    ...
    func unitCard(for unit: UnitEntry) -> some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            HStack(alignment: .center, spacing: 12) {
                VStack(alignment: .leading, spacing: 4) {
                    Text("不正解 \(unit.totalIncorrectCount) 問")
                        .font(.subheadline.weight(.semibold))
                        .foregroundColor(.themeIncorrect)

                    Text("不正解だった章数: \(unit.chapters.count)")
                        .font(.caption)
                        .foregroundColor(.themeTextSecondary)
                }
    ...
    func setRootHeader() {
        ...
        mainViewState.setHeader(title: "不正解だった問題", backButton: backButton)
    }
...
extension IncorrectAnswerView {
    struct UnitEntry: Identifiable, Hashable {
        ...
        var totalIncorrectCount: Int {
            chapters.reduce(into: 0) { $0 += $1.incorrectCount }
        }
    ...
    struct ChapterEntry: Identifiable, Hashable {
        ...
        var incorrectCount: Int { questions.count }

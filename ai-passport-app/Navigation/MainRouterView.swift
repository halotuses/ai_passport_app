     ...
     var body: some View {
         NavigationStack(path: $router.path) {  // ← router.path をバインド（reset()が効く）
             Group {
                ...
                } else if let unit = mainViewState.selectedUnit, mainViewState.selectedChapter == nil {
                     // 章一覧
                    ChapterListView(unit: unit, viewModel: chapterListVM, selectedChapter: $mainViewState.selectedChapter)
